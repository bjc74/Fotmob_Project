...
def validate_scores(competition_id, season_id, n=20):
    ...
    for _, match in matches.head(n).iterrows():
        match_id = match["match_id"]

        try:
            report = match_report(match_id)

            predicted_score = report["score"]
            ...
            for team, official_goals in official_score.items():
                predicted_goals = predicted_score.get(team)

                if predicted_goals != official_goals:
                    errors.append({
                        "match_id": match_id,
                        "team": team,
                        "predicted": predicted_goals,
                        "official": official_goals,
